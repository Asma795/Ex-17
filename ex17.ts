...
while (guestLeast.length > 2) {
  let removed = guestLeast.pop();
  console.log(
    `Hello Dear ${removed}, We are really sorry that we had to cancel your invitation.\n`
  );
}
...
for (let i = 0; i < guestLeast.length; i++) {
  console.log(
    `Hello Dear ${guestLeast[i]}, You are still invited to join us.\n`
  );
}
